...
const TAG_RULES = [
  ...
  { tag: "crypto", patterns: [/crypto|bitcoin|btc|eth|token|blockchain|polygon|usdc/i] },
  ...
  // Content & Social
  { tag: "x-twitter", patterns: [/twitter|tweet|x\.com|twikit/i] },
  { tag: "content", patterns: [/content|digest|newsletter|article|post/i] },
  { tag: "outreach", patterns: [/outreach|listing|mexc|exchange|bd\b/i] },

  // Projects
  { tag: "tokengotchi", patterns: [/tokengotchi|gotchi|sprite|pet/i] },
  { tag: "vibe-island", patterns: [/vibe.?island|island/i] },
  { tag: "lottery", patterns: [/lottery|乐透|lotto/i] },
  { tag: "dating", patterns: [/dating|tinder|bumble|hinge/i] },
  { tag: "voice", patterns: [/voice|speech|tts|speak|daemon/i] },
...
function discoverFiles() {
  ...
  const pyBot = join(HOME, "polymarket-bot");
  if (existsSync(pyBot)) {
    walkDir(pyBot, files, "code-python", 3);
  }

  // Prediction markets — TypeScript
  const tsBot = join(HOME, "prediction-markets", "packages");
  if (existsSync(tsBot)) {
    walkDir(tsBot, files, "code-typescript", 4);
  }
