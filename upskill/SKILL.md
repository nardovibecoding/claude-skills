...
7. **Extract subroutine** (`scripts/extract.py`) — only fires on `decision=adopt`. Clones repo to `/tmp/upskill_extract_*`, runs (a) `~/.claude/skills/upskill/scripts/skill_security_auditor.py` (security gate, folded in 2026-04-30 from retired `skill-security-auditor` skill), then (b) **discipline-quality screen** per `references/discipline-quality-screen.md` (added 2026-05-01 — 17 yes/no checks against active disciplines D1-D16), then (c) **intent-vs-implementation gap screen** (added 2026-05-02 — runs `python3 ~/.claude/hooks/comment_code_audit.py --diff <last-N-commits>` against the candidate's recent diff; non-empty findings reduce the discipline score by 1 point per finding, max -5). Combined verdict: PASS-INSTALL (security PASS + ≥14/17 disciplines + no HIGH-$ violations + gap-score≤2) → install to `~/.claude/skills/<slug>/`, EXTRACT-only (security PASS + 10-13 disciplines, no HIGH-$ violations) → write to `~/NardoWorld/atoms/extracted-patterns/<slug>-<date>.md`, REJECT (security FAIL OR <10 disciplines OR any HIGH-$ violation OR gap-score>5) → abort. Append final ledger row + discipline-receipt entries.

   **Why screen (c) exists:** label-vs-code drift (2026-05-02 vps_sync.sh P2 incident) is a candidate-quality signal. A repo whose comment blocks promise more than the code enforces is shipping a skill we'd have to re-audit at install time. The 17-discipline screen catches structural issues; the gap screen catches honesty issues.
