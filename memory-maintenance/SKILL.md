...
## When to run
- Automatically via VPS cron (daily 04:30 HKT)
- Manually via /memory-maintenance
- After any major debugging session or architecture change

---

## Part 1: Extract from expiring JSONL

1. Find JSONL files older than 7 days (Mac or VPS)
2. Read existing memory index: `cat ~/telegram-claude-bot/memory/MEMORY.md`
3. For each JSONL: extract head 100 + tail 300 lines, grep for text content
4. Look for: architecture decisions, new features, bug root causes, user corrections, cron changes, config changes
...
2. For each project/reference memory, verify claims against current state:
   - Cron schedules: `crontab -l` on VPS vs what memory says
...
1. Delete scanned JSONL files (>7 days)
...
Identify patterns in MEMORY.md and topic files that should graduate to enforced rules in CLAUDE.md or `.claude/rules/`. Auto-memory captures well but has no judgment about what is temporary vs permanent.
...
- Same concept appears in 2+ memory files or 2+ places in MEMORY.md
- User has corrected Claude about this same thing in multiple sessions
- The pattern matches a feedback_*.md file (already an explicit user preference)
...
| Personal preference | `~/.claude/CLAUDE.md` | "Use orange icons for admin UI" |
| Already a feedback_*.md | `.claude/rules/` with path scope | Move from soft memory to hard rule |
...
2. Remove the original entry from MEMORY.md / topic file
3. If a feedback_*.md was promoted, note in the feedback file that it's now an enforced rule
4. Verify CLAUDE.md stays under 200 lines; overflow to `.claude/rules/`
5. Report: what was promoted, where it lives, lines freed in MEMORY.md
...
| feedback_*.md not yet promoted | 0 | 1-3 | > 3 |
...
2. **Name** using lowercase-hyphen format, 2-4 words describing the problem (e.g., `docker-arm64-fix`, `ssh-tunnel-debug`)
